...
void classifySegments(double ** dist, double *slopeOfLines, double **overlap, int n, Array ** classifiedLines)
{
  // FILE * classified;
  // classified = fopen("classified_segments.txt","w");

  *classifiedLines = (Array *)(malloc(sizeof( Array )* n));
  Array * innerLines = *classifiedLines;
  for(int outer=0; outer<n; outer++) //points to frame - line to compare with
  {
    //printf("start %d \n",outer);
    Array * sameLine = (Array *)malloc(sizeof(Array ) );
    initArray(sameLine, 50);//first entry stores all similar lines
    insertArray(sameLine, outer);
    //initArray(&sameLine[1], 50);//second entry stores all parallel lines
    ...
    for(int inner=0; inner<n; inner++) // points to ref - line who is compared with frame
    {
      ...
      int t2=100; // distThreshold1
      int t3=0; // distThreshold2
      int t4=5;//overlap distance threshold
      ...
      //For outer = 0 , lets print all closest pairs  in one image  based on conditions
      if(overlap[outer][inner] != -1 && abs(slopeF-slopeR) <= 5 && distFromFrameToRef < t2 && overlapDistFromFrameToRef == 0)
        insertArray(sameLine, inner);
       // if(overlap[outer][inner] != -1 && abs(slopeF-slopeR) == 0 && (distFromFrameToRef >t3 && distFromFrameToRef < t2) && overlapDistFromFrameToRef > 0)
       //  insertArray(sameLine, inner);
    }
    innerLines[outer] = *sameLine;
  }
  //printf("done \n");
}
...
void findOverLapDistance(double ** dist, Array * innerLines, double * slopeOfLines, double * segs, int cur, double w, double h, int num_segs)
{
  ...
  for(int outer=0;outer<innerLines->used;outer++) //we are looping through all lines that are parallel to cur index line
  {
    //outer index is just to iterate through the parallel lines array, dont use it for any thing else .

    int lineNum = innerLines->array[outer]; //lineNum is nothing but array index num that was stored to look up in segs n*5 d array
    ...
    //points 2894 (5017.796397,4508.583200) (5005.967515,5005.967515)
      ...
      if(slopeOfLines[cur] >=0)
      {
        int frame_s_X1 = segs[cur*7+0] >= segs[cur*7+2] ? segs[cur*7+2] : segs[cur*7+0];//s means start
        int frame_e_X2 = segs[cur*7+0] >= segs[cur*7+2] ? segs[cur*7+0] : segs[cur*7+2];//e means end
        int ref_s_X3 = segs[lineNum*7+0] >= segs[lineNum*7+2] ? segs[lineNum*7+2] : segs[lineNum*7+0];
        int ref_e_X4 = segs[lineNum*7+0] >= segs[lineNum*7+2] ? segs[lineNum*7+0] : segs[lineNum*7+2];;
      ...
      else //these lines are oriented to Y axis , note we have not checked for slope infinity lines but thos lines may not appear
      {
        int frame_s_Y1 = segs[cur*7+1] >= segs[cur*7+3] ? segs[cur*7+3] : segs[cur*7+1];//s means start
        int frame_e_Y2 = segs[cur*7+1] >= segs[cur*7+3] ? segs[cur*7+1] : segs[cur*7+3];//e means end
        int ref_s_Y3 = segs[lineNum*7+1] >= segs[lineNum*7+3] ? segs[lineNum*7+3] : segs[lineNum*7+1];
        int ref_e_Y4 = segs[lineNum*7+1] >= segs[lineNum*7+3] ? segs[lineNum*7+1] : segs[lineNum*7+3];
        ...
        if( frame_s_Y1 >= ref_s_Y3 && frame_e_Y2 <= ref_e_Y4) //it means frame is between and smaller than reference line seg
          d = fabs((frame_e_Y2 - frame_s_Y1)/cos(thetaFrame));
        else if( frame_s_Y1 <= ref_s_Y3 && frame_e_Y2 >= ref_e_Y4) //it means ref is between and smaller than frame line seg
          d = fabs((ref_e_Y4 - ref_s_Y3)/cos(thetaRef));
        //means they may some what overlap and don't at all - we need to consider 2 combinations of ref and frame arrangement
        else if(frame_s_Y1 <= ref_e_Y4)
          d = fabs((ref_e_Y4 - frame_s_Y1)/cos(thetaFrame));
        else if(frame_e_Y2 >= ref_s_Y3)
          d = fabs((ref_s_Y3 - frame_e_Y2)/cos(thetaRef));
        //-1.0 means there is no overlap
...
void findParallelLines(Array ** parallelLines , double * segs, int n, int X, int Y, int dim, double * slopeOfLines)
{
  printf("Enter findParallelLines \n");
  *parallelLines = (Array *)(malloc(sizeof(Array )* n));
  ...
  for(int outer = 0; outer <n ; outer++)
  {
    ...
    for(int inner=0; inner <n ; inner++)
    {
      ...
      if(abcissa_1 == abcissa_2) //it means its parallel
      {
...
int main(int argc, char **argv)
{
  ...
  //printf("points 0 (%f,%f) (%f,%f)",out[0*dim+0],out[0*dim+1],out[0*dim+2],out[0*dim+2]);
  //sort based on line segment distance 
  qsort(out , n, sizeof(double) * dim, cmpfunc);

  //printf("points 2894 (%f,%f) (%f,%f)",out[2894*dim+0],out[2894*dim+1],out[2894*dim+2],out[2894*dim+3]);
  //printf("points 0 (%f,%f) (%f,%f)",out[0*dim+0],out[0*dim+1],out[0*dim+2],out[0*dim+3]);

  //find parallel lines
  Array * parallelLines = NULL;
  double *slopeOfLines = calloc(n, sizeof(double));
  findParallelLines(&parallelLines, out, n, X, Y, dim, slopeOfLines);
  ...
  Array * allGroupedLineSegments = NULL;
  classifySegments(distArray, slopeOfLines, overlapArray, n, &allGroupedLineSegments);
  //Array * first = &allGroupedLineSegments[0];
  //Array * second = &allGroupedLineSegments[1];
  for(int ni =0; ni< n; ni++) {
    char str[50]={0};
    sprintf(str,"singles/%d-.eps",ni);
    write_eps_BS(&allGroupedLineSegments[ni],out, ni, dim, str, X, Y, 2.0, ni);  
  }
